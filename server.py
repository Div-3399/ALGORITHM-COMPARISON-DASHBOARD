...
async def upload(file: UploadFile = File(...)):
    try:
        ...
        # Extract unique items and encode in one-hot format
        ...
        # Seaborn style for consistent charts
        ...
        for size in scalability_sizes:
            ...
            fpg_time, _, _ = execute_fpgrowth(subset)
            apr_time, _, _ = execute_apriori(subset)
            scalability_metrics["FP-Growth"].append(fpg_time)
            scalability_metrics["Apriori"].append(apr_time)

        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(scalability_sizes, scalability_metrics["FP-Growth"], label="FP-Growth",
                color='#5a4fcf', marker='o', linestyle='-', linewidth=2)
        ax.plot(scalability_sizes, scalability_metrics["Apriori"], label="Apriori",
                color='#7b6dff', marker='o', linestyle='-', linewidth=2)
...
# # Detect dense format: items are comma-separated in one column
# if 'items' in columns:
#     transactions = df['items'].astype(str).str.split(', ')
# # Detect grouped format: e.g., 'membernumber', 'date', 'itemdescription'
# elif set(['membernumber', 'date', 'itemdescription']).issubset(columns):
#     df['transaction_id'] = df['membernumber'].astype(str) + "_" + df['date'].astype(str)
#     transactions = df.groupby('transaction_id')['itemdescription'].apply(list)
# # If dataset has only one unnamed column
# elif len(columns) == 1:
#     transactions = df[df.columns[0]].astype(str).str.split(', ')
# else:
#     raise ValueError("Unrecognized dataset format. Expecting either a single-column of comma-separated items, or membernumber/date/itemdescription structure.")

# # Binarize transactions into a one-hot encoded dataframe
# unique_items = sorted(set(item for sublist in transactions for item in sublist))
# binary_encoded = pd.DataFrame(False, index=range(len(transactions)), columns=unique_items)
